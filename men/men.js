...
mensData.forEach((mendata) => {
  let card = document.createElement("div");
  let image = document.createElement("img");
  image.src = mendata.image_url;

  let description = document.createElement("p");
  description.textContent = mendata.name;

  let price = document.createElement("p");
  price.textContent = mendata.price;

  let addToCart = document.createElement("button");
  addToCart.textContent = "Add To Cart";

  addToCart.addEventListener("click", () => {
    cartsData.push(mendata);
    localStorage.setItem("cartsData", JSON.stringify(cartsData));
    alert("item added to cart");
  });

  card.append(image, description, price, addToCart);
